...
local function lock_group_links(msg, data, target)
  ...
  local hash = 'group:'..msg.to.id
    ...
    if group_lang then 
    return 'قفل لینک گروه فعال شده بود'
	  else
...
local function lock_group_all(msg, data, target)
  ...
  local hash = 'group:'..msg.to.id
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'همه قفل ها فعال شده بودند'
	  else
...
local function unlock_group_all(msg, data, target)
  ...
  local hash = 'group:'..msg.to.id
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'همه قفل ها غیر فعال بوده اند'
  else
    ...
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'همه قفل ها غیر فعال شده اند'
    else
...
local function unlock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'no' then
    ...
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'قفل خارج شدن غیر فعال است'
	  else
    ...
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'قفل خارج شدن غیر فعال شد'
	  else
    return 'leave has been unlocked'
  end
end
local function lock_group_fun(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_fun_lock = data[tostring(target)]['settings']['fun']
  if group_fun_lock == 'yes' then
	    save_data(_config.moderation.data, data)
	  local hash = 'group:'..msg.to.id
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'قفل دستورات فان فعال است'
	else
    return 'help fun is already locked'
	end
	end
    data[tostring(target)]['settings']['fun'] = 'yes'
    save_data(_config.moderation.data, data)
	    save_data(_config.moderation.data, data)
	  local hash = 'group:'..msg.to.id
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'قفل دستورات فان فعال شد'
	else
    return 'help fun has been locked'
  end
end

local function unlock_group_fun(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_fun_lock = data[tostring(target)]['settings']['fun']
  if group_fun_lock == 'no' then
	    save_data(_config.moderation.data, data)
	  local hash = 'group:'..msg.to.id
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'همه قفل ها غیر فعال است'
	else
    return 'help fun is not locked'
 end
 end
    data[tostring(target)]['settings']['fun'] = 'no'
    save_data(_config.moderation.data, data)
	    save_data(_config.moderation.data, data)
	  local hash = 'group:'..msg.to.id
    local group_lang = redis:hget(hash,'lang')
    if group_lang then 
    return 'قفل دستورات فلن غیر فعال شد'
	else
    return 'help fun has been unlocked'
  end
end
...
function show_supergroup_settingsmod(msg, target)
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['fun'] then
			data[tostring(target)]['settings']['fun'] = 'no'
		end
	end
	if data[tostring(target)]['settings'] then
	...
	local data = load_data(_config.moderation.data)
		...
		unlock_group_operator(msg, data, target),
      	...
      	return lock_group_etehad(msg, data, target), etehad
			...
			if matches[2] == 'helpfun' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked help fun ")
				return lock_group_fun(msg, data, target)
			end			
		...
		unlock_group_operator(msg, data, target),
      	...
      	return unlock_group_etehad(msg, data, target), detehad
			...
			if matches[2] == 'helpfun' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked help fun")
				return unlock_group_fun(msg, data, target)
			end
			if matches[2] == 'emoji' then
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "Message /superhelp to @SceneManager in private for SuperGroup help\nYou can send /helpfun in SuperGroup\n Channel ~ @TGland"
